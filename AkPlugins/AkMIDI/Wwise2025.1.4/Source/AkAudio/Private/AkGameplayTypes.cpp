...
#pragma region H3DWwise	
	if (FAkAudioDevice::Get() && FAkAudioDevice::Get()->GetAkCallbackInfoPool()) 
	{
#pragma endregion
...
#pragma region H3DWwise	
	...
	{
		UE_LOG(LogAkAudio, Error, TEXT("[AkAudio] UAkEventCallbackInfo::Create Failed, Pool is null, playingID = %d, eventID = %d"), AkEventCbInfo->playingID,AkEventCbInfo->eventID);
		UAkComponent* AkComponent = UAkComponent::GetAkComponent(AkEventCbInfo->gameObjID);
		if (AkComponent && AkComponent->GetOwner()) 
		{
			if (AkComponent->AkAudioEvent) 
			{
				UE_LOG(LogAkAudio, Error, TEXT("[AkAudio] UAkEventCallbackInfo::Create Failed, Pool is null, Name = %s"), *AkComponent->AkAudioEvent.GetName());
			}
		}
	}	
	return nullptr;
