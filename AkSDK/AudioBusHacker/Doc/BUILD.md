...
以下四个文件必须位于同一目录。`bundle.json` 是入口，三个 `.tar.xz` 是它引用的数据包：
...
不要单独改名、移动或解压某个数据包，也不要使用 `LegacyPackages` 中的 2023.1 包。当前 bundle 仅适用于 Wwise 2025.1、Windows x64 和 `vc170`。

| 数据包 | 用途 | 一般是否安装 |
| --- | --- | --- |
| Authoring Release | Wwise 设计工具插件、XML 和 Factory Asset | 是 |
| Authoring Debug | 调试版 Wwise Authoring 使用 | 仅调试 Authoring 时 |
| SDK Windows_vc170 | 游戏运行时 DLL、静态库、PDB 和 Factory 头 | 开发及集成游戏时 |

### 10.2 通过 Wwise Launcher 安装

这是推荐方式：

1. 关闭所有 Wwise Authoring 实例。
2. 打开 Audiokinetic Launcher，找到 Wwise `2025.1.4.9062` 的插件管理入口。
3. 选择 **Add Plug-ins / Install New Plug-ins**，再选择本地 bundle、离线包或本地目录。不同 Launcher 版本的文字可能略有差异。
4. 选择**包含 `bundle.json` 的目录**，不要选择单个 `.tar.xz`。
5. 正常使用 Wwise 时安装 Authoring Release；集成游戏时同时安装 SDK Windows_vc170；只有使用 Debug Authoring 时才安装 Authoring Debug。
6. 确认目标安装版本为 `G:\Wwise2025.1.4.9062`，然后执行安装。
7. 启动 Wwise，在 Effect/效果器列表中确认可以创建 `AudioBusHacker`。

安装后检查：

```powershell
$WwiseRoot = "G:\Wwise2025.1.4.9062"
$InstalledFiles = @(
    ...
    "$WwiseRoot\Authoring\Data\Factory Assets\AudioBusHacker\Manifest.xml"
)
Get-Item $InstalledFiles | Select-Object FullName, Length, LastWriteTime
```

### 10.3 手工安装

Launcher 不可用时，可将 Release Authoring 包和 SDK 包直接展开到 Wwise 根目录。包内已有 `Authoring/...` 和 `SDK/...` 相对路径，所以解压目标必须是 Wwise 根目录：

```powershell
$BundleDir = "H:\m2_wwise_test01\v_wwise\YXK_AkMIDI\AkSDK\AudioBusHacker"
$WwiseRoot = "G:\Wwise2025.1.4.9062"
$Xz = Join-Path $WwiseRoot "Tools\Win32\bin\xz.exe"
$Packages = @(
    "AudioBusHacker_v2025.1.4_Build9062_Authoring.Windows_x64.Release.tar.xz",
    "AudioBusHacker_v2025.1.4_Build9062_SDK.Windows_vc170.tar.xz"
)
...
foreach ($Package in $Packages) {
    & $Xz -dc (Join-Path $BundleDir $Package) |
        tar -xf - -C $WwiseRoot
...
如需调试 Wwise Authoring，再额外展开 Debug Authoring 包。手工安装不会在 Launcher 中登记，升级和卸载也必须手工管理。

### 10.4 游戏运行时接入

Authoring 包只负责让 Wwise 设计工具识别插件。游戏还必须包含 SoundEngine 插件，选择以下一种方式：

**静态链接（推荐）**

1. 链接与游戏配置一致的 `AudioBusHackerFX.lib`：Debug、Profile、Release 分别使用 `SDK\x64_vc170\<Config>\lib`。
2. 使用静态 CRT 时，改用对应 `Debug(StaticCRT)`、`Profile(StaticCRT)` 或 `Release(StaticCRT)` 目录。
3. 在插件注册编译单元中包含：
...
该 Factory 头会注册 `AudioBusHackerFX`，并声明 `SetAudioBusHackerCallbacks`。

**动态加载**

把匹配配置的 `SDK\x64_vc170\<Config>\bin\AudioBusHacker.dll` 放入当前引擎/Wwise 集成使用的动态插件搜索目录，并确认 SoundEngine 已启用动态插件加载。具体目录由引擎和集成方式决定。

静态链接和动态加载不要同时使用。游戏、Wwise SDK、插件、架构、配置和 CRT 必须保持一致。

### 10.5 验证与更新

安装后应确认：

1. Wwise 能创建 `AudioBusHacker`，Placeholder 属性可编辑。
2. 生成 SoundBank 时没有插件缺失错误。
3. 游戏加载 SoundBank 时没有插件 ID `15450` 缺失或版本不兼容错误。
4. 注册回调后，`SetAudioBusHackerCallbacks` 能收到音频缓冲回调。

若重新构建后仍使用相同 bundle ID/版本，Launcher 可能把它视为已安装版本。应先卸载旧插件再安装，或为正式发布递增 `year.major.minor.build` 版本。
