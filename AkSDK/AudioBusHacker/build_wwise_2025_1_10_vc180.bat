...
:preflight
echo ============================================================
echo AudioBusHacker - Wwise 2025.1.10.9233 - vc180 / x64
...
set "VERIFY_FAILED=0"
set "SDK_PACKAGE=%PROJECT_ROOT%AudioBusHacker_v2025.1.10_Build9233_SDK.Windows_vc180.tar.xz"
...
call %PYTHON_LAUNCHER% -c "import json; b=json.load(open(r'%PROJECT_ROOT%bundle.json', encoding='utf-8')); v=b['version']; names={f.get('sourceName', f.get('id', '')) for f in b['files']}; prefix='AudioBusHacker_v2025.1.10_Build9233_'; required='AudioBusHacker_v2025.1.10_Build9233_SDK.Windows_vc180.tar.xz'; raise SystemExit(0 if (v['year'],v['major'],v['minor'],v['build']) == (2025,1,10,9233) and required in names and all(n.startswith(prefix) for n in names) else 1)"
if errorlevel 1 (
    echo [ERROR] bundle.json version or required vc180 package is incorrect.
    set "FAILED_STEP=Verify bundle.json"
    exit /b 1
)

echo [OK] bundle.json version and required vc180 package reference are correct.
